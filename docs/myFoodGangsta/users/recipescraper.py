...
def recipeScrape(url):
    ...
    for index in scraper.ingredients():
        if index.count(" ") == 1:
            ...
            temp = index.split(" ", 2)
            ingredients = [temp[0] + " " + temp[1], temp[2]]
        else:
            ingredients = []
    ...
    for i in range(len(listIngre)):
        if len(listIngre[i]) == 2:
            strIngre = strIngre + "- " + listIngre[i][0] + " " + listIngre[i][1] + "\n"
        elif len(listIngre[i]) == 1:
            strIngre = strIngre + "- " + listIngre[i][0] + "\n"
        else:
            strIngre = strIngre + "- Unknown ingredient format\n"
    ...
    recipeDict = {
        ...
        "url": url  # Add the recipe URL here
    }
